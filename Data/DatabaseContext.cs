...
namespace WOWStore.Data
{
    public class DatabaseContext : IAsyncDisposable
    {
        ...
        public async Task<bool> AddAllItemsAsync<TTable>(List<TTable> items) where TTable : class, new()
        {
            await CreateTableIfNotExists<TTable>();
            //return await Database.InsertAsync(item) > 0;
            return await Execute<TTable, bool>(async () => await Database.InsertAllAsync(items) > 0);
        }
